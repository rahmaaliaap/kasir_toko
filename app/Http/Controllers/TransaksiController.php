...
{
    // Lakukan pengecekan stok barang di sini
    // Misalnya, dengan mengambil data dari database atau dari sumber lain

    $namaBarang = $request->input('nama_barang');
    // Contoh sederhana: mengecek stok barang dari array stokBarang
    $stokBarang = [
        "Buku" => 10,
        "Pensil" => 20,
        // Tambahkan barang lain di sini dengan jumlah stoknya
    ];

    if (array_key_exists($namaBarang, $stokBarang)) {
        return response()->json(['message' => 'Stok barang mencukupi'], 200);
    } else {
